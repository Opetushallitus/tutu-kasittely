...
export const MuuPerustelu = ({
  perustelu: maybePerustelu,
  updatePerustelu,
}: Props) => {
  const { t } = useTranslations();

  const [currentValue, setValue] = useState(maybePerustelu?.muuPerustelu || '');

  const updatePerusteluWithValue = (newValue: string) => {
    const newPerustelu = { muuPerustelu: newValue };
    setValue(newValue);
  ...
  return (
    <OphInputFormField
      multiline={true}
      data-testid={`yleiset-perustelut__muu-perustelu`}
      label={t('hakemus.perustelu.yleiset.perustelut.muuPerustelu')}
      value={currentValue}
