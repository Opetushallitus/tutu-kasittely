import { Table, TableContainer, TableHead, TableRow } from '@mui/material';
import { useQueryClient } from '@tanstack/react-query';
import { parseAsString, useQueryState } from 'nuqs';
import * as R from 'remeda';

import StyledTableBody from '@/src/app/components/StyledTableBody';
import TableSortLabel from '@/src/app/components/TableSortLabel';
import MessageRow from '@/src/app/yhteinenKasittely/MessageRow';
import { User } from '@/src/lib/types/user';
import { YhteisenKasittelynViesti } from '@/src/lib/types/yhteisenKasittelynViesti';
import { setYkViestilistaQueryState } from '@/src/lib/utils';

const FIELD_KEYS = {
  lahetetty: 'lahetetty',
  tila: 'tila',
  hakija: 'hakija',
  asiatunnus: 'asiatunnus',
};

export default function SentMessages({
  messageList,
  user,
}: {
  messageList: YhteisenKasittelynViesti[] | null;
  user: User | null;
}) {
  const queryClient = useQueryClient();
  const [sortDef, setSortDef] = useQueryState('sort', {
    ...parseAsString.withDefault('tila:asc'),
    clearOnDefault: false,
  });
  const messageRows =
    messageList && user
      ? R.map(messageList, (message, index) => {
          return (
            <MessageRow message={message} sentMessage={true} key={index} />
          );
        })
      : [];

  const handleSort = async (sortDef: unknown) => {
    await setYkViestilistaQueryState(
      queryClient,
      setSortDef,
      sortDef,
      'getYkSentMessages',
    );
  };

  return (
    <TableContainer>
      <Table>
        <TableHead>
          <TableRow>
            {R.map(Object.values(FIELD_KEYS), (fieldKey) => (
              <TableSortLabel
                key={fieldKey}
                fieldKey={fieldKey}
                sortDef={sortDef}
                handleSort={handleSort}
              />
            ))}
          </TableRow>
        </TableHead>
        <StyledTableBody data-testid={'lahetetyt-viestit-list'} tabIndex={0}>
          {messageRows}
        </StyledTableBody>
      </Table>
    </TableContainer>
  );
}
