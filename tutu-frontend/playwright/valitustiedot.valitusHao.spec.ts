...
test('HaO valitettu, valintaruutu näyttää ja piilottaa päivämääräkentät, ja tallentaa muutokset', async ({
  page,
}) => {
  const valitettuCheckbox = page.getByTestId('valitushao-valitettu-checkbox');
  const valitusPvmCalendar = page.getByTestId('valitushao-valituspvm-calendar');
  const ratkaisuPvmCalendar = page.getByTestId(
    'valitushao-ratkaisupvm-calendar',
  );
  ...
  expect(requestBody.valitusHaO.valitettu).toBe(false);
  expect(requestBody.valitusHaO.valitusPvm).toBeUndefined();
  expect(requestBody.valitusHaO.ratkaisuPvm).toBeUndefined();

  await expect(valitettuCheckbox).not.toBeChecked();
  await expect(valitusPvmCalendar).toBeHidden();
});

test('HaO ratkaisupäivä on lukittu kunnes valituspäivä on valittu', async ({
  page,
}) => {
  await page.getByTestId('valitushao-valitettu-checkbox').click();

  const valitusPvmInput = page
    .getByTestId('valitushao-valituspvm-calendar')
    .locator('input');
  const ratkaisuPvmInput = page
    .getByTestId('valitushao-ratkaisupvm-calendar')
    .locator('input');
...
test('HaO valitusPvm tyhjentäminen tyhjentää myös ratkaisuPvm', async ({
  page,
}) => {
  await page.getByTestId('valitushao-valitettu-checkbox').click();
...
test('HaO:n ratkaisu, radiovalinnat näytetään jos valitettu, valinta tallentuu ja on tyhjennettävissä', async ({
  page,
}) => {
  const ratkaisuRadioGroup = page.getByTestId(
    'valitushao-ratkaisu-radio-group',
  );
  const ratkaisuClearButton = page.getByTestId(
    'valitushao-ratkaisu-radio-group-clear-button',
  );
  ...
  await expectRequestData(
    ...
    '/valitustiedot',
    ratkaisuRadioGroup
      .locator('input[type="radio"][value="UudelleenKasittely"]')
      .click(),
    {
      valitusHaO: { ratkaisu: 'UudelleenKasittely' },
    },
  );

  await expect(
    ratkaisuRadioGroup.locator(
      'input[type="radio"][value="UudelleenKasittely"]',
    ),
  ).toBeChecked();
  ...
  await expectRequestData(
    ...
    '/valitustiedot',
    ratkaisuRadioGroup
      .locator('input[type="radio"][value="KasittelyRauennut"]')
      .click(),
    {
      valitusHaO: { ratkaisu: 'KasittelyRauennut' },
    },
  );
  ...
  await expect(
    ratkaisuRadioGroup.locator(
      'input[type="radio"][value="KasittelyRauennut"]',
    ),
  ).toBeChecked();
  ...
  expect(clearRequest.postDataJSON().valitusHaO.ratkaisu).toBeUndefined();
...
test('HaO:n lisätietokenttä näytetään kun ratkaisu on valittu, tallentuu ja tyhjenee kun ratkaisun poistaa', async ({
  page,
}) => {
  const ratkaisuRadioGroup = page.getByTestId(
    'valitushao-ratkaisu-radio-group',
  ...
  const ratkaisuLisatietoInput = page.getByTestId(
    'valitushao-ratkaisulisatieto-input',
  );
  ...
  const requestBody = clearRequest.postDataJSON();
  expect(requestBody.valitusHaO.ratkaisu).toBeUndefined();
  expect(requestBody.valitusHaO.ratkaisuLisatieto).toBeUndefined();

  await expect(ratkaisuLisatietoInput).toBeHidden();
});

test('HaO:n ratkaisu piilotetaan ja tyhjennetään kun valitus poistetaan', async ({
  page,
}) => {
  const valitettuCheckbox = page.getByTestId('valitushao-valitettu-checkbox');
  const ratkaisuRadioGroup = page.getByTestId(
