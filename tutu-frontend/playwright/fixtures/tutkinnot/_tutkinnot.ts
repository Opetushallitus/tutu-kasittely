import { Tutkinto } from '@/src/lib/types/tutkinto';

export const _tutkinnot: Tutkinto[] = [
  {
    id: '18732268-07ca-4898-a21f-e49b9dd68275',
    hakemusId: '3f140ba6-4018-402c-af32-5e5b802144fc',
    jarjestys: '1',
    nimi: 'Päälikkö',
    nimiAlkuperaiskielella: 'Head of Department',
    nimiKaannoksessa: 'Osastopäällikkö',
    oppilaitos: 'Butan Amattikoulu',
    oppilaitosAlkuperaiskielella: 'Bhutan Vocational School',
    oppilaitosKaannoksessa: 'Butanin ammattikoulu',
    aloitusVuosi: 1999,
    paattymisVuosi: 2000,
    maakoodiUri: 'maatjavaltiot2_762',
    muuTutkintoTieto: undefined,
    todistuksenPaivamaara: undefined,
    koulutusalaKoodiUri: undefined,
    paaAineTaiErikoisala: undefined,
    todistusOtsikko: 'tutkintotodistus',
  },
  {
    id: '589038c5-00eb-465b-98bf-3b9ce62bb94d',
    hakemusId: '3f140ba6-4018-402c-af32-5e5b802144fc',
    jarjestys: '2',
    nimi: 'Apu poika',
    oppilaitos: 'Apu koulu',
    aloitusVuosi: 2010,
    paattymisVuosi: 2011,
    maakoodiUri: 'maatjavaltiot2_762',
    muuTutkintoTieto: undefined,
    todistuksenPaivamaara: undefined,
    koulutusalaKoodiUri: undefined,
    paaAineTaiErikoisala: undefined,
    todistusOtsikko: 'muutodistus',
  },
  {
    id: '07f503b7-7cf4-4437-b4c6-97512bd44450',
    hakemusId: '3f140ba6-4018-402c-af32-5e5b802144fc',
    jarjestys: 'MUU',
    nimi: undefined,
    nimiAlkuperaiskielella: undefined,
    nimiKaannoksessa: undefined,
    oppilaitos: undefined,
    oppilaitosAlkuperaiskielella: undefined,
    oppilaitosKaannoksessa: undefined,
    aloitusVuosi: undefined,
    paattymisVuosi: undefined,
    maakoodiUri: undefined,
    muuTutkintoTieto: 'En olekaan suorittanutkoulutusta',
    todistuksenPaivamaara: undefined,
    koulutusalaKoodiUri: undefined,
    paaAineTaiErikoisala: undefined,
    todistusOtsikko: undefined,
    muuTutkintoMuistio: 'HUOMIO!',
  },
];
