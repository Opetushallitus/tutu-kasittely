...
class BooleanFieldWrapperTest:
  ...
  @Test
  def testFromPartialWithSomeTrue(): Unit =
    val wrapper = BooleanFieldWrapper(Some(true))
    val result  = BooleanFieldWrapper.fromPartial(Some(wrapper))
    assertTrue(result.value.contains(true), "fromPartial should extract Some(true)")

  @Test
  def testFromPartialWithSomeFalse(): Unit =
    val wrapper = BooleanFieldWrapper(Some(false))
    val result  = BooleanFieldWrapper.fromPartial(Some(wrapper))
    assertTrue(result.value.contains(false), "fromPartial should extract Some(false)")

  @Test
  def testFromPartialWithNone(): Unit =
    val wrapper = BooleanFieldWrapper(None)
    val result  = BooleanFieldWrapper.fromPartial(Some(wrapper))
    assertTrue(result.value.isEmpty, "fromPartial should extract None")

  @Test
  def testFromPartialWithNoWrapper(): Unit =
    val result = BooleanFieldWrapper.fromPartial(None)
    assertTrue(result.value.isEmpty, "fromPartial with None should return None")
