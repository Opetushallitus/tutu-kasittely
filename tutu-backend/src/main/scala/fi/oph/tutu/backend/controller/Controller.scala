...
class Controller(
  ...
  module.addDeserializer(classOf[JatkoOpintoKelpoisuusWrapper], new JatkoOpintoKelpoisuusWrapperDeserializer())
    ...
    try {
      ...
      if (!AuthoritiesUtil.hasTutuAuthorities(authorities)) {
        ...
        if (!hakemus.hakemusOid.isValid) {
          ...
          val (hakemusId, perustelu, paatos) = hakemusService.luoKokonainenHakemus(
            hakemus,
            PartialPerustelu(
