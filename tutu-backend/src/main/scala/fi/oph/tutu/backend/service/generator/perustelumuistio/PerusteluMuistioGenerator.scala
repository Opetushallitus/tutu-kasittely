...
def haeHakemusKoskeeRivit(kieli: Kieli, item: Option[SisaltoItem], level: Int = 0): Seq[(Int, String, String)] = {
  if (item.isEmpty) {
    ...
    val alirivit: Seq[(Int, String, String)] =
      allChildren.flatMap(child => haeHakemusKoskeeRivit(kieli, Option(child), level + 1))
    ...
    case Some(perustelu) =>
      val resultString = Seq(
        ...
        perustelu.ylimmanTutkinnonAsemaLahtomaanJarjestelmassa
          ...
          .map(muotoiltuAsema => s"Ylimmän tutkinnon asema lähtömaan järjestelmässä: $muotoiltuAsema"),
...
def haeRatkaisutyyppi(paatos: Paatos): Option[String] = {
  paatos.ratkaisutyyppi
    ...
    .map((muotoiltu: String) => s"Ratkaisutyyppi: $muotoiltu")
}
