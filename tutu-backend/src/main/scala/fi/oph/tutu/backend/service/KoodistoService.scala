...
@Component
@Service
class KoodistoService(oauth2Client: Oauth2Client, httpService: HttpService, maakoodiService: MaakoodiService)
  ...
  def getKoodistoRelaatiot(koodiUri: String): Either[Throwable, String] = {
    ...
    httpService.get(oauth2Client, url)
  }
  ...
  private def haeKorkeakouluTyypinOppilaitokset(oppilaitostyyppiUri: String): Set[String] = {
    ...
    httpService.get(oauth2Client, url) match {
