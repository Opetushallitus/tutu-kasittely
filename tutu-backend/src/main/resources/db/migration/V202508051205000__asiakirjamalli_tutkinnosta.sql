...
CREATE TABLE IF NOT EXISTS asiakirjamalli_tutkinnosta (
    ...
    CONSTRAINT fk_asiakirjamalli_tutkinnosta_hakemus FOREIGN KEY (hakemus_id) REFERENCES hakemus(id)
    );
...
COMMENT ON COLUMN asiakirjamalli_tutkinnosta.hakemus_id IS 'Hakemustaulun hakemuksen id';
...
CREATE UNIQUE INDEX idx_asiakirjamalli_tutkinnosta_hakemus_lahde ON asiakirjamalli_tutkinnosta(hakemus_id, lahde);
